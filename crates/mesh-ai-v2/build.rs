...
fn main() {
    println!("cargo:rerun-if-changed=cuda/brain_v2_train.cu");
    println!("cargo:rerun-if-env-changed=MESH_BRAIN_CUDA");
    println!("cargo:rerun-if-env-changed=MESH_REQUIRE_CUDA");
    println!("cargo:rustc-check-cfg=cfg(mesh_brain_cuda)");

    let force_off = env::var("MESH_BRAIN_CUDA").ok().as_deref() == Some("0");
    if force_off {
        println!("cargo:warning=MESH_BRAIN_CUDA=0 — brain v2 CUDA disabled");
        return;
    }

    let out = PathBuf::from(env::var("OUT_DIR").unwrap());
    let cu = PathBuf::from("cuda/brain_v2_train.cu");
    let obj = if cfg!(windows) {
        out.join("brain_v2_train.obj")
    } else {
        out.join("brain_v2_train.o")
    };
    ...
    match status {
        Ok(s) if s.success() => {
            let lib_name = "mesh_brain_v2_cuda";
            ...
            if !lib_ok {
                let require = env::var("MESH_REQUIRE_CUDA").ok().as_deref() == Some("1")
                    || env::var("MESH_BRAIN_CUDA").ok().as_deref() == Some("1");
                let msg = "could not package mesh_brain_v2_cuda static lib";
                if require {
                    panic!("{msg}");
                }
            ...
            println!("cargo:rustc-cfg=mesh_brain_cuda");
        }
        Ok(s) => {
            let require = env::var("MESH_REQUIRE_CUDA").ok().as_deref() == Some("1")
                || env::var("MESH_BRAIN_CUDA").ok().as_deref() == Some("1");
            let msg = format!(
                "brain v2 nvcc failed (exit {:?}) via {} — CPU-only",
                s.code(),
                nvcc.display()
            );
            if require {
        ...
        Err(e) => {
            let require = env::var("MESH_REQUIRE_CUDA").ok().as_deref() == Some("1")
                || env::var("MESH_BRAIN_CUDA").ok().as_deref() == Some("1");
            let msg = format!(
                "brain v2 nvcc not runnable ({e}) via {} — CPU-only",
                nvcc.display()
            );
            if require {
                panic!("{msg}");
